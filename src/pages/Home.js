...
function Home(props) {
  ...
  async function getListedNFTs() {
    ...
    await results.forEach((listedNFTs) => {
      const { owner, tokenId, price, amount, createdAt } = listedNFTs.attributes;
      listings.push({
        id: listedNFTs.id,
        owner,
        tokenId,
    ...
    console.log('+++++ imageURIs A +++++++', imageURIs)
  ...
  async function getNFTs(tokenIds) {
    ...
    console.log('+++++ here 2 +++++++', readOptions)
    try{
      const imageURIs = await Moralis.executeFunction(readOptions);
      console.log('+++++ here 3 +++++++', imageURIs)
      return imageURIs;
    }catch(e){
      console.log('e', e)
      return null;
  ...
  return (
    <section className="relative">
      ...
      <div className="max-w-5xl mx-auto lg:flex ">
        <div
          className={`flex-[0.3] lg:block ${isSidebar ? "block px-3" : "hidden"
